...
def get_knob_preset_scheduler(config: PilotConfig) -> PilotScheduler:
    ...
    scheduler.register_collect_data("llamatune_data", pull_execution_time=True)
    # TimeStatistic.print()
    # start
    scheduler.init()
    return scheduler
...
def get_knob_spark_preset_scheduler(config: PilotConfig) -> PilotScheduler:
    ...
    scheduler.register_collect_data("llamatune_data_spark", pull_execution_time=True)
    # allow to pretrain model
    periodic_db_controller_event = KnobPeriodicDbControllerEvent(config, 2000,
                                                                 llamatune_config_file="../examples/KnobTuning/llamatune/configs/llama_config_spark.ini",
                                                                 exec_in_init=True, optimizer_type="smac")
    scheduler.register_event(EventEnum.PERIODIC_DB_CONTROLLER_EVENT, periodic_db_controller_event)
